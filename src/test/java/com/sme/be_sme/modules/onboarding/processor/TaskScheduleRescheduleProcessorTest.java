...
class TaskScheduleRescheduleProcessorTest {
    ...
    @Test
    void reschedule_updatesStatusAndReason() {
        TaskScheduleRescheduleProcessor processor = new TaskScheduleRescheduleProcessor(
                new ObjectMapper(),
                taskInstanceMapper,
                activityLogService,
        ...
        assertEquals(OnboardingTaskWorkflow.SCHEDULE_RESCHEDULED, response.getScheduleStatus());
        assertEquals("busy", response.getScheduleRescheduleReason());
        verify(activityLogService).logScheduleRescheduled(any(), any(), any());
        verify(workflowNotificationService).notifyScheduleRescheduled(any(), any(), any());
    }
}
